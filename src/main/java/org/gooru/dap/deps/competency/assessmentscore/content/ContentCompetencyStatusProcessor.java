...
public class ContentCompetencyStatusProcessor {
	...
	public void process() {
		...
		if (score != null && score >= COMPLETION_SCORE) {
			LOGGER.debug("Content Competency Status: competency:{} || status=completed", this.competencyCode);
		...
		// completed. This should be handled in the query at DAO layer.
		LOGGER.debug("Content Competency Status: competency:{} || status=inprogress", this.competencyCode);
