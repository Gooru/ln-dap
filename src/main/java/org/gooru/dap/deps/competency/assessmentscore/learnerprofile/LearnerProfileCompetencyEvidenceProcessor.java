...
public class LearnerProfileCompetencyEvidenceProcessor {
	...
	/*
	 * For TS tables even if we are persisting multiple evidence for the same
	 * competency, however, if the competency is already COMPLETED or MASTERED then
	 * there is no point in persisting evidence for INPROGRESS again. In this case
	 * we should avoid persisting the evidence.
	 */
	private void processCompetencyForTS(LearnerProfileCompetencyEvidenceCommand command, int status) {
		...
		boolean isCompletedOrMastered = this.service.checkIfCompetencyIsAlreadyCompletedOrMastered(bean);
	...
	private void processMicroCompetencyForTS(LearnerProfileCompetencyEvidenceCommand command, int status) {
		...
		boolean isCompletedOrMastered = this.service
				.checkIfMicroCompetencyIsAlreadyCompletedOrMastered(microCompetencyBean);
		LOGGER.debug("micro competency '{}' ||  isCompletedOrMastered:{}", gutCode, isCompletedOrMastered);
		if (status == StatusConstants.IN_PROGRESS) {
			if (!isCompletedOrMastered) {
