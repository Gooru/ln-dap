...
public abstract class LearnerProfileCompetencyStatusDao {
   ...
   * 0 = Not started
   * 
   * 1 = in_progress (If there is any collection/assessment that are started and NO SINGLE
   * assessment for that competency is completed as yet)
   * 
   * 2 = inferred (This is based on the progression data. A competency is inferred (as mastered) if
   * the competency above it is Mastered by the user)
   * 
   * 3 = NA cannot be derived for now.
   * 
   * 4 = completed (If there is any collection/assessment that are started and ANY SINGLE
   * assessment(s) for that competency is completed (i.e score is available, received
   * collection.play (.stop) event)
   * 
   * 5 = mastered (If there is any collection/assessment that are started and ANY SINGLE
   * assessment(s) for that competency is completed and the SCORE is >80%)
   */
