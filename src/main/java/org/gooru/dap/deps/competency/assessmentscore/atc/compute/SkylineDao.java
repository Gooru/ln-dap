...
interface SkylineDao {
	   ...
	   List<CompetencyModel> fetchUserSkyline (@Bind("user") String user, @Bind("subjectCode") String subjectCode);
	   
	   @Mapper(CompetencyModelMapper.class)
	   @SqlQuery("select dcm.tx_domain_code, dcm.tx_comp_code, dcm.tx_comp_seq, lps.status from domain_competency_matrix dcm "
	   		+ " LEFT JOIN learner_profile_competency_status lps ON dcm.tx_comp_code=lps.gut_code where "
	   		+ "lps.user_id = :user and dcm.tx_subject_code = :subjectCode order by dcm.tx_domain_code, dcm.tx_comp_seq asc")
	   List<CompetencyModel> fetchUserDomainCompetencyStatus (@Bind("user") String user, @Bind("subjectCode") String subjectCode);
