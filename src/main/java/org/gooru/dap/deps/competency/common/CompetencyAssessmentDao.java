...
public abstract class CompetencyAssessmentDao {
  ...
  protected abstract AssessmentCompetency getAssessmentCompetency(
      @Bind("assessmentId") String assessmentId);
  ...
  @SqlQuery("SELECT competency_gut_code FROM signature_items WHERE item_id = :assessmentId AND item_format = 'assessment'")
  protected abstract List<String> fetchSignatureAssessment(
      @Bind("assessmentId") String assessmentId);
}
