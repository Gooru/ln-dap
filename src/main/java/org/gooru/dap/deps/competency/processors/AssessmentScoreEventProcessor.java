...
public class AssessmentScoreEventProcessor implements EventProcessor {
	 ...
	 * 
	 * Update - 01/08/2018 : Lookup for signature gut codes of assessment only if
	 * path id is present in the event. Else skip the signature items lookup. If
	 * signature assessment, then no need to use the competencies tagged with the
	 * assessment to mark the status. Just use gut codes from signature_items table
	 * to be mark the status.
	 */
	private void processAssessmentScore() {
		...
		if (pathId == null || pathId == 0) {
			...
			LOGGER.debug("'{}' gut codes found in signature assessment for '{}'", signatureGutCodes.size(),
					collectionId);
