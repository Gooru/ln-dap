...
import org.gooru.dap.deps.competency.assessmentscore.atc.competency.CompetencyPath;
...
import org.gooru.dap.deps.competency.assessmentscore.atc.competency.ProgressionLevel;
...
class GradeCompetencyModelBuilder {
    ...
    GradeCompetencyModel build(SubjectCode subjectCode, CompetencyCollector competencyCollector) {
        ...
        domainsAsString = competencyCollector.getDomains().stream().map(DomainCode::getCode).collect(Collectors.toList());
    ...
    private void enrichCompetencyWithDetails() {
        ...
        for (DomainModel domainModel : domains) {
            CompetencyPath competencyPath = competencyCollector.getPathForDomain(domainModel.getDomainCode());
