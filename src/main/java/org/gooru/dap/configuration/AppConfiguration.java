...
/**
 * This class is supposed to make use of {@link Config} class and provide the required configuration
 * to caller. The idea of putting this class is to make sure that different hard code keys for
 * configuration are not scattered across different source files, rather they are confined to one
 * class, which is this one.
 *
...
public final class AppConfiguration {
  ...
  public static JsonNode fetchJobsConfig() {
    return Config.getInstance().getRootConfig().get("jobs.config");
  }
